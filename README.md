...
The argoCD applications include a `platofrm` config that (after manual sync) installs an Azure ProviderConfig. That config requires a Kubernetes seceret per the instructions here [Azure Provider Secret Conmfig](https://github.com/upbound/platform-ref-azure#configure-providers-in-your-platform). Future iterations of this repo will inclide other cloud provider option and respective ProviderConfig instructions.
